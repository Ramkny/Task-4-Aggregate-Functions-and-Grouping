...
GROUP BY Members.Name;
 ...
 Count total number of books by each author
...
GROUP BY a.AuthorID;
 ...
 SELECT m.Name AS MemberName, COUNT(l.LoanID) AS TotalLoans
FROM Members m
JOIN Loans l ON m.MemberID = l.MemberID
GROUP BY m.MemberID;

-- 3. Find average number of books borrowed per member
...
-- 4. List authors who have more than 2 books in the library
SELECT a.Name AS AuthorName, COUNT(b.BookID) AS TotalBooks
FROM Authors a
JOIN Books b ON a.AuthorID = b.AuthorID
GROUP BY a.AuthorID
...
--  5. Count number of books currently borrowed (not returned)
SELECT COUNT(*) AS CurrentlyBorrowed
FROM Loans
WHERE ReturnDate IS NULL;

-- 6. Count number of loans per month
SELECT 
...
-- 7. Find members who borrowed more than 3 books
